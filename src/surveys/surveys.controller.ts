...
export class SurveysController {
  ...
  @Post()
  @ApiOperation({ summary: 'Create a survey template (creator is taken from the token)' })
  @ApiResponse({ status: 201, description: 'Survey created' })
  @ApiResponse({ status: 400, description: 'Invalid input' })
  create(@Body() dto: CreateSurveyDto, @CurrentUser() currentUser: { id: bigint }) {
  ...
  @ApiResponse({ status: 400, description: 'Invalid input' })
  @ApiResponse({ status: 404, description: 'Survey not found' })
  update(@Param('id', ParseBigIntPipe) id: bigint, @Body() dto: UpdateSurveyDto) {
    return this.surveysService.update(id, dto);
  }

  @Delete(':id')
  @HttpCode(204)
  @ApiOperation({ summary: 'Delete a survey template that has no versions' })
  @ApiParam({ name: 'id', type: String, example: '1' })
  @ApiResponse({ status: 404, description: 'Survey not found' })
  @ApiResponse({ status: 409, description: 'Survey has versions and cannot be deleted' })
